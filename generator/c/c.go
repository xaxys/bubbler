...
func (g *CGenerator) generateType(type_ definition.Type) (def string, method string) {
...
func (g *CGenerator) generateStruct(structDef *definition.Struct) (def string, method string) {
	methodsStr := ""
	fieldsStr := ""
	startBits := int64(0)
			...
			case *definition.Array:
				...
				if option, ok := val.FieldOptions.Get("order"); ok {
					if option.OptionValue.GetLiteralValue() == "big" {
						// 大端序
						fieldData = func(i int64) string {
							return fmt.Sprintf("((*(uint%d_t*)(&((structPtr->%s)[%d]))) >> %d)", tySize*8, val.FieldName, i/tySize, (fieldSize%ty.Length-i%tySize-1)*8)
						}
					}
				}
				if fieldData == nil {
...
func (g *CGenerator) generateMethods(structDef *definition.Struct, fieldDef *definition.NormalField) string {
	...
	rawGetStr := genGet("GetRaw_"+fieldDef.FieldName, fieldDef.FieldType, &definition.ValueExpr{})
	rawSetStr := genSet("SetRaw_"+fieldDef.FieldName, fieldDef.FieldType, &definition.ValueExpr{})
	str += rawGetStr + "\n"
	str += rawSetStr + "\n"
...
func (g *CGenerator) generateEnum(enumDef *definition.Enum) string {
