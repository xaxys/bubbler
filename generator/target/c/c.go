...
#include "{{ $curUnit.Package.ToRelativePathStrict $unit.Package ".bb.h" }}"
{{ else -}}
...
{{- define "structConst" -}}
static const bool {{ .StructDef.StructName }}_dynamic = {{ if .StructDef.StructDynamic }}true{{ else }}false{{ end }};
...
{{- define "decoderDecl" -}}
{{- $structName := .StructDef.StructName -}}
// DecoderSizeDecl: {{ $structName }}_decode_size
            ...
            {{- if $field.FieldType.GetTypeID.IsArray }}
                ...
                {{- if $field.FieldType.ElementType.GetTypeID.IsStruct }}
                    {{- if $field.FieldType.ElementType.GetTypeDynamic }}
                        {{- if $shouldUseLoop }}
    for (int64_t _i = 0; _i < {{ $field.FieldType.Length }}; _i++) {
        size += {{ $field.FieldType.ElementType.GetTypeName }}_encode_size(&ptr->{{ $fieldName }}[_i]);
    }
                        {{- else }}
                        {{- range $i := iterate 0 $field.FieldType.Length }}
    size += {{ $field.FieldType.ElementType.GetTypeName }}_encode_size(&ptr->{{ $fieldName }}[{{ $i }}]);
                        {{- end }}
                        {{- end }}
                    {{- end }}
                {{- else if $field.FieldType.ElementType.GetTypeID.IsString }}
                    {{- if $shouldUseLoop }}
    for (int64_t _i = 0; _i < {{ $field.FieldType.Length }}; _i++) {
        size += strlen(ptr->{{ $fieldName }}[_i]) + 1;
    }
                    {{- else }}
                    {{- range $i := iterate 0 $field.FieldType.Length }}
    size += strlen(ptr->{{ $fieldName }}[{{ $i }}]) + 1;
                    {{- end }}
                    {{- end }}
                {{- else if $field.FieldType.ElementType.GetTypeID.IsBytes }}
                    {{- if $shouldUseLoop }}
    for (int64_t _i = 0; _i < {{ $field.FieldType.Length }}; _i++) {
        size += ptr->{{ $fieldName }}[_i].length;
        size += 1{{ range $j := iterate 1 10 }} + (bool)(ptr->{{ $fieldName }}[_i].length >> {{ calc $j "*" 7 }}){{ end }};
    }
                    {{- else }}
                    {{- range $i := iterate 0 $field.FieldType.Length }}
    size += ptr->{{ $fieldName }}[{{ $i }}].length;
    size += 1{{ range $j := iterate 1 10 }} + (bool)(ptr->{{ $fieldName }}[{{ $i }}].length >> {{ calc $j "*" 7 }}){{ end }};
                    {{- end }}
                    {{- end }}
                {{- end }}
            {{- else if $field.FieldType.GetTypeID.IsStruct }}
                {{- if $field.FieldType.GetTypeDynamic }}
    size += {{ $field.FieldType.GetTypeName }}_encode_size(&ptr->{{ $fieldName }});
                {{- end }}
            {{- else if $field.FieldType.GetTypeID.IsString }}
    size += strlen(ptr->{{ $fieldName }}) + 1;
            {{- else if $field.FieldType.GetTypeID.IsBytes }}
    size += ptr->{{ $fieldName }}.length;
    size += 1{{ range $j := iterate 1 10 }} + (bool)(ptr->{{ $fieldName }}.length >> {{ calc $j "*" 7 }}){{ end }};
            {{- end }}
        {{- end }}
    {{- end }}
    return size;
...
{{- define "encodeNormalFieldStruct" -}}
    {{ if .FieldStruct.GetTypeDynamic }}offset += {{ end }}{{ .FieldStruct.GetTypeName }}_encode(&{{ .FieldName }}, (({{ .TyUint8 }}*)data) + {{ if .Dynamic }}offset + {{ end }}{{ .FromByte }});
{{- end -}}
...
{{- define "encodeNormalFieldString" -}}
    {
        uint64_t {{ .TempName }} = strlen({{ .FieldName }});
        memcpy((({{ .TyUint8 }}*)data) + offset + {{ .FromByte }}, {{ .FieldName }}, {{ .TempName }});
        (({{ .TyUint8 }}*)data)[offset + {{ .FromByte }} + {{ .TempName }}] = 0;
        offset += {{ .TempName }} + 1;
    }
{{- end -}}
...
{{- $fieldDataName := printf "(%s).data" .FieldName -}}
    {
        ...
        do (({{ .TyUint8 }}*)data)[offset + {{ .FromByte }}] = (({{ .TempName }} & {{ .GetMask }}) | {{ .SetMask }}), {{ .TempName }} >>= {{ .Shift }}, offset++; while ({{ .TempName }});
        (({{ .TyUint8 }}*)data)[offset - 1 + {{ .FromByte }}] &= ~{{ .SetMask }};
        memcpy(({{ .TyUint8 }}*)data + offset + {{ .FromByte }}, {{ $fieldDataName }}, {{ $fieldLengthName }});
...
func (g CGenerator) generateEncodeNormalField(field *definition.NormalField, startBits int64) ([]string, error) {
	...
	case *definition.Array:
		...
		if shouldUseLoop && useDynamicLoop {
			...
			loopStmts = append(loopStmts, fmt.Sprintf("for (int64_t _i = 0; _i < %d; _i++) {", ty.Length))
...
func (g CGenerator) generateEncodeNormalFieldImpl(fieldNameStr string, fieldType definition.Type, fieldOptions *util.OrderedMap[string, *definition.Option], structDynamic bool, from, to int64) ([]string, error) {
	encodeStmts := []string{}
...
func (g CGenerator) generateEncodeImpl(from, to int64, fieldData func(int64) string, structDynamic bool) []string {
	encodeStmts := []string{}
...
{{- $structName := .StructDef.StructName -}}
{{- $structBytes := calc .StructDef.StructBitSize "/" 8 -}}
// DecoderSize: {{ $structName }}_decode_size
// Returns the encoded size (> 0) if successful, 
// or the negative minimum required size (< 0) if data is insufficient.
// The required size may change as more data is provided.
{{ if .GenOptions.SingleFile }}static {{ end -}}
...
for (int64_t _i = 0; _i < {{ .Length }}; _i++) {
    if ({{ .StructName }}_decode({{ .DataExpr }}, &((({{ .ArrayName }})[_i]))) < 0) return -1;
}
{{- end -}}
...
func (g CGenerator) generateDecodeNormalFieldImpl(fieldNameStr string, fieldType definition.Type, fieldOptions *util.OrderedMap[string, *definition.Option], structDynamic bool, from, to int64) ([]string, error) {
	decodeStmts := []string{}
	fieldBitSize := to - from
...
func (g CGenerator) generateDecodeImpl(from, to int64, fieldProcessor func(string, int64) string, dataData func(int64) string) []string {
	decodeStmts := []string{}
