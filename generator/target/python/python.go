...
var fileTemplate = `
{{- define "runtimeHelpers" -}}
def _bb_write_field_bits(data: bytearray, bit_from: int, bit_size: int, value: int, big_endian: bool) -> None:
	if bit_size <= 0:
		return
	value &= (1 << bit_size) - 1
    ...
    def encode_size(self) -> int:
                ...
                {{- if $field.FieldType.GetTypeID.IsArray }}
                    ...
                    {{- if $field.FieldType.ElementType.GetTypeID.IsStruct }}
                        {{- if $field.FieldType.ElementType.GetTypeDynamic }}
                            {{- if $shouldUseLoop }}
        for _i in range({{ $field.FieldType.Length }}):
            size += self._{{ $fieldName }}[_i].encode_size()
                            {{- else }}
        ...
        size += self._{{ $fieldName }}[{{ $i }}].encode_size()
                    ...
                    {{- else if $field.FieldType.ElementType.GetTypeID.IsString }}
                        {{- if $shouldUseLoop }}
        for _i in range({{ $field.FieldType.Length }}):
            size += len(bytes(self._{{ $fieldName }}[_i], 'utf-8')) + 1
                        {{- else }}
                        {{- range $i := iterate 0 $field.FieldType.Length }}
        size += len(bytes(self._{{ $fieldName }}[{{ $i }}], 'utf-8')) + 1
                    ...
                    {{- else if $field.FieldType.ElementType.GetTypeID.IsBytes }}
                        {{- if $shouldUseLoop }}
        for _i in range({{ $field.FieldType.Length }}):
            size += len(self._{{ $fieldName }}[_i])
            size += 1{{ range $j := iterate 1 10 }} + bool(len(self._{{ $fieldName }}[_i]) >> {{ calc $j "*" 7 }}){{ end }}
                        {{- else }}
                        {{- range $i := iterate 0 $field.FieldType.Length }}
...
{{ .Body }}
{{- else if eq .Kind "fixed_loop" -}}
for _i in range({{ .Length }}):
{{ .Body }}
{{- end -}}
{{- end -}}

{{- define "encodeFieldBits" -}}
	_bb_write_field_bits(data, {{ .BitBaseExpr }}, {{ .FieldBitSize }}, {{ .FieldValueExpr }}, {{ .BigEndian }})
{{- end -}}

{{- define "encodeStructFieldLoop" -}}
	{{ if .FieldStruct.GetTypeDynamic }}offset += {{ end }}{{ .FieldName }}.encode(data[{{ .ByteBaseExpr }}:])
{{- end -}}
...
func (g PythonGenerator) generateEncodeNormalField(field *definition.NormalField, startBits int64) ([]string, error) {
	...
	case *definition.Array:
		...
		if shouldUseLoop && useDynamicLoop {
			...
			var subNameFunc func(string) string
			switch ty.ElementType.(type) {
			case *definition.Struct, *definition.BasicType, *definition.String, *definition.Bytes:
				subNameFunc = func(loopVar string) string {
					return fmt.Sprintf("%s[%s]", name, loopVar)
				}
			case *definition.Enum:
				tempName := g.generateEncodeTempVarName(startBits)
				elementNameStr := fmt.Sprintf("%s[_i]", name)

				encodeNormalFieldTempVarAssignData := map[string]any{
					"TempName":  tempName,
					"FieldName": elementNameStr,
				}
				...
				subNameFunc = func(_ string) string {
					return tempName
				}
			...
			// Generate the loop body for one iteration with loop variable _i
			subFrom := from
			subTo := from + elemBitSize

			subName := subNameFunc("_i")
			...
			// Fixed-size element arrays use real bit-base loop math.
			bodyStmts := []string{}
			...
			switch elemTyType := ty.ElementType.(type) {
			case *definition.Struct:
				structLoopData := map[string]any{
					"FieldStruct":  elemTyType,
					"FieldName":    fmt.Sprintf("%s[_i]", name),
					"ByteBaseExpr": fmt.Sprintf("(%s) // 8", bitBaseExpr),
				}
				stmt := util.ExecuteTemplate(fieldEncoderTemplate, "encodeStructFieldLoop", nil, structLoopData)
				bodyStmts = append(bodyStmts, util.IndentSpace(stmt, 4))

			case *definition.Enum:
				encodeFieldBitsData := map[string]any{
					"BitBaseExpr":    bitBaseExpr,
					"FieldBitSize":   elemBitSize,
					"FieldValueExpr": fmt.Sprintf("%s[_i].value", name),
					"BigEndian":      pyBoolLiteral(gen.MatchOption(field.FieldOptions, "order", "big")),
				}
				stmt := util.ExecuteTemplate(fieldEncoderTemplate, "encodeFieldBits", nil, encodeFieldBitsData)
				bodyStmts = append(bodyStmts, util.IndentSpace(stmt, 4))

			case *definition.BasicType:
				fieldValueExpr := fmt.Sprintf("%s[_i]", name)
				if elemTyType.GetTypeID().IsFloat() {
					tyID := typeMapStructPackID[elemTyType.GetTypeID()]
					tyUintID := typeMapStructPackID[typeSizeMapUint[elemTyType.GetTypeBitSize()]]
					fieldValueExpr = fmt.Sprintf("struct.unpack('<%s', struct.pack('<%s', %s[_i]))[0]", tyUintID, tyID, name)
				}
			...
			default:
				return nil, fmt.Errorf("internal error: unsupported fixed-loop array element type %T", ty.ElementType)
			}
...
{{- define "decodeFieldBits" -}}
    {{ .TempName }} = _bb_read_field_bits(data, {{ .BitBaseExpr }}, {{ .FieldBitSize }}, {{ .BigEndian }})
{{- end -}}
...
func (g PythonGenerator) generateDecodeNormalField(field *definition.NormalField, startBits int64) ([]string, error) {
	...
	case *definition.Array:
		...
		if shouldUseLoop && useDynamicLoop {
			...
			tempName := g.generateDecodeTempVarName(startBits) + "_elem"
			...
			case *definition.Struct:
				...
				stmt := util.ExecuteTemplate(fieldDecoderTemplate, "decodeStructFieldLoop", nil, structLoopData)
				bodyStmts = append(bodyStmts, util.IndentSpace(stmt, 4))

			case *definition.Enum:
				decodeFieldBitsData := map[string]any{
					"TempName":     tempName,
					"BitBaseExpr":  bitBaseExpr,
					"FieldBitSize": elemBitSize,
					"BigEndian":    pyBoolLiteral(gen.MatchOption(field.FieldOptions, "order", "big")),
				}
				stmt := util.ExecuteTemplate(fieldDecoderTemplate, "decodeFieldBits", nil, decodeFieldBitsData)
				bodyStmts = append(bodyStmts, util.IndentSpace(stmt, 4))
				bodyStmts = append(bodyStmts, fmt.Sprintf("    %s[_i] = %s(%s)", name, elemTyType.EnumName, tempName))
			...
			case *definition.BasicType:
				...
				if elemTyType.GetTypeID().IsInt() {
					signMask := uint64(1) << (elemBitSize - 1)
					bodyStmts = append(bodyStmts, fmt.Sprintf("    %s = ((%s) ^ %s) - %s", tempName, tempName, g.generateHex(signMask), g.generateHex(signMask)))
				}

				if elemTyType.GetTypeID().IsFloat() {
					tyID := typeMapStructPackID[elemTyType.GetTypeID()]
					tyUintID := typeMapStructPackID[typeSizeMapUint[elemTyType.GetTypeBitSize()]]
					bodyStmts = append(bodyStmts, fmt.Sprintf("    %s = struct.unpack('<%s', struct.pack('<%s', %s))[0]", tempName, tyID, tyUintID, tempName))
				}

				bodyStmts = append(bodyStmts, fmt.Sprintf("    %s[_i] = %s", name, tempName))
