...
type PythonSingleGenerator struct {
...
func (g *PythonSingleGenerator) Generate(ctx *genctx.GenCtx) error {
	g.GenTypes = util.NewOrderedMap[string, *GeneratedType]()
	g.GenStack = util.NewOrderedMap[string, any]()

	unit := ctx.Units[0]

	for _, type_ := range unit.GlobalTypes.Values() {
...
func (g *PythonSingleGenerator) generateType(type_ definition.Type) (def string) {
...
func (g *PythonSingleGenerator) generateTypeDefaultValue(type_ definition.Type) (def string) {
	switch val := type_.(type) {
	case *definition.Struct:
...
func (g *PythonSingleGenerator) generateStruct(structDef *definition.Struct) *GeneratedType {
...
func (g *PythonSingleGenerator) generateStructMethods(structDef *definition.Struct) string {
	funcMap := template.FuncMap{
...
func (g *PythonSingleGenerator) generateExpr(expr definition.Expr, valueReplacement string) string {
	funcMap := template.FuncMap{
		"generateExpr": g.generateExpr,
		"generateType": g.generateType,
	}
...
func (g *PythonSingleGenerator) generateEnum(enumDef *definition.Enum) *GeneratedType {
...
func (g *PythonSingleGenerator) generateEncoder(structDef *definition.Struct) string {
...
func (g *PythonSingleGenerator) generateDecoder(structDef *definition.Struct) string {
