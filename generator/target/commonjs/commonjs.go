...
{{- $structName := .StructDef.StructName -}}
                ...
                {{ $structName }}.encode_size = function(obj) {
                    ...
                    for (let _i = 0; _i < {{ $field.FieldType.Length }}; _i++) {
                        size += obj.{{ $fieldName }}[_i].encode_size();
                    }
                                        {{- else }}
                    ...
                    size += obj.{{ $fieldName }}[{{ $i }}].encode_size();
                                ...
                                {{- else if $field.FieldType.ElementType.GetTypeID.IsString }}
                                    {{- if $shouldUseLoop }}
                    for (let _i = 0; _i < {{ $field.FieldType.Length }}; _i++) {
                        size += stringToUTF8BytesCount(obj.{{ $fieldName }}[_i]) + 1;
                    }
                                    {{- else }}
                                    {{- range $i := iterate 0 $field.FieldType.Length }}
...
{{- $structBytes := calc .StructDef.StructBitSize "/" 8 -}}
                 ...
                 * @description Returns the encoded size (> 0) if successful, 
                 ...
                 */
                {{ $structName }}.decode_size = function(data, start) {
                    if (start === undefined) start = 0;
                    {{- if .StructDef.StructDynamic }}
                    var offset = 0;
                    {{- $fixedStart := 0 }}
                 ...
                 * @instance
                 */
                {{ $structName }}.prototype.decode_size = function(data, start) {
...
func (g CommonJSGenerator) GenerateDecoder(structDef *definition.Struct) (string, error) {
	...
	decoderStr := util.ExecuteTemplate(decoderTemplate, "decoder", funcMap, fieldData)
