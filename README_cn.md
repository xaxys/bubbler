...
**警告**：UTF-8 编码是一种约定。在 C 和 C++ 目标中，生成代码**不处理** UTF-8 编码。用户需要**自行确保**字符串值是 UTF-8 编码的。但在 Go、Java、Python、JS 等其他现代语言中，UTF-8 编码由生成代码处理。（不过如果你只使用 C/C++，你可以忽略此约定并使用任何编码，只要字符串的结尾由 `\0` 表示即可。）
