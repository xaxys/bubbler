...
var matrixTargets = []matrixTarget{
	...
	},
	{
		...
		BubblerID:  "kotlin",
		SkipEnv:    "SKIP_KOTLIN",
		Tools:      []string{"kotlinc", "java"},
		LoopMarker: "for (_i in 0 until",
		Options: []matrixVariant{
			{Name: "decnum", Flags: []string{"-decnum"}},
			{Name: "signext_arith", Flags: []string{"-signext=arith"}},
		},
...
func (runtime *matrixRuntime) runVariant(target matrixTarget, variant matrixVariant, dir string) error {
	...
	case "kotlin":
		return runtime.runKotlin(target, variant, dir)
	case "python":
...
func (runtime *matrixRuntime) runKotlin(target matrixTarget, variant matrixVariant, dir string) error {
	genDir := filepath.Join(dir, "gen")
	if err := os.MkdirAll(genDir, 0o755); err != nil {
		return err
	}
	...
	if err := writeMatrixFile(mainFile, EmitKotlin(runtime.spec)); err != nil {
		return err
	}
	sources, err := filesWithSuffix(genDir, ".kt")
	if err != nil {
		return err
	}
	sources = append(sources, mainFile)
	jar := filepath.Join(dir, "tests.jar")
	args := append([]string{}, sources...)
	args = append(args, "-jvm-target", "17", "-include-runtime", "-d", jar)
	if err := runCommand(dir, "kotlinc", args...); err != nil {
		return err
	}
	return runCommand(dir, "java", "-jar", jar)
}
...
func isGeneratedSource(path string) bool {
	...
	switch ext {
	case ".c", ".cpp", ".go", ".java", ".kt", ".cs", ".js", ".py":
